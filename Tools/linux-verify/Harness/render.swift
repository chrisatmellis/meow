...
///
/// The cat arrives here already posed, for the same reason. `CatSkin` blends the
/// skeleton into these very arrays, so a picture of the cat is a picture of what
/// the phone draws. It used to draw the bind pose instead — one still animal in
/// every shot, whatever the animator had done — which is precisely why a skinning
/// bug could not be seen here and reached a device twice.
